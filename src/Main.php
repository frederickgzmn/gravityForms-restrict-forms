...
class Main
{
	 ...
	 */
	private static $AttemptLimit = 10;
	...
	/**
	 * @var string Ip addresses can't be blocked
	 */
	public static $ips_from_olimpo = [];
	...
	private function GFRestrictIPCount () {
		...
		$rowcount = $wpdb->get_var("SELECT count FROM {$gfrestrictip_table_name} WHERE ip = '{$ip}' and CURDATE() = day_date ");
	...
	private function GFRestrictIPAddCount () {
		...
		if ( in_array( $ip, self::$ips_from_olimpo ) ) {
			return true;
		}

		if ( $countQly === false ) {
			$return = $wpdb->query( "INSERT INTO {$gfrestrictip_table_name} (ip, count, day_date)
    		VALUES('{$ip}', 1, CURDATE() )" );
		} else {
